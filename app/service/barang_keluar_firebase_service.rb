...
class BarangKeluarFirebaseService
  def self.all
    response = FIREBASE_CLIENT.get("barang_keluar")
  ...
  def self.find(cargo_id)
    response = FIREBASE_CLIENT.get("barang_keluar/#{cargo_id}")
  ...
  def self.create(attributes)
    ...
    response = FIREBASE_CLIENT.set("barang_keluar/#{cargo_id}", attributes)

...
class BarangMasukFirebaseService
  def self.all
    response = FIREBASE_CLIENT.get("barang_masuk")
  ...
  def self.find(cargo_id)
    response = FIREBASE_CLIENT.get("barang_masuk/#{cargo_id}")
  ...
  def self.create(attributes)
    ...
    response = FIREBASE_CLIENT.set("barang_masuk/#{cargo_id}", attributes)
